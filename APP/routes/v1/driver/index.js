    ...
    *}
    ...
    api.get('/getStopList', function (req, res) {
        ...
        req.asyncValidationErrors().then(function () {
            ...
            client.query("select order_id,driver_id,DATE_FORMAT(delivery_time,'%H:%i') as delivery_time,DATE_FORMAT(delivery_time,'%H:%i') as delivered_time,pickup_location,drop_location,drop_location_lat,drop_location_lang,pickup_location_lat,pickup_location_lang,order_date as order_date,users.first_name,users.last_name,users.profile_pic,order_status from driver_assigned_order left join users on driver_assigned_order.user_id=users.id  where driver_id=? order by order_status desc", [data.driver_id], function (err, result) {
                if (err)
                {
                    Util.makeResponse(res, false, 500, "Something went wrong",appVersion,[]);
                }
   ...
   **/
   ...
   api.post('/logout', function (req, res) {
    ...
    api.get('/getLaunchApi', function (req, res) {
        ...
        req.checkQuery(schema);
        req.asyncValidationErrors().then(function () {
     client.query("select shift_id,driverAvgRating('"+data.driver_id+"') as driver_avg_rating from shift_clock where driver_id=? and date=?",[data.driver_id,date], function (err, result) {
                if (err)
                {
                    Util.makeResponse(res, false, 500, "Something went wrong",appVersion, []);
                }
